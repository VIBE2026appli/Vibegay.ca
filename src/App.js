...
function Home({ displayName, onLogout }) {
  ...
  return (
    <div style={{
      ...
      {/* ⚡ Bolt: Forced heavy CSS filters onto a separate GPU layer using translateZ(0) and will-change to avoid main-thread repaint lag */}
...
function AuthenticatedApp({ user, displayName, city, identity, view, setView, onLogout }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: T.dark }}>
      ...
      {/* ⚡ Bolt: Forced heavy CSS backdrop filter onto a separate GPU layer using translateZ(0) and will-change to avoid main-thread repaint lag */}
      <nav style={{
