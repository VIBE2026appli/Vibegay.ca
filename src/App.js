...
function Home({ displayName, onLogout }) {
  ...
  return (
    <div style={{
      ...
      {/* ⚡ Bolt: Hardware accelerate heavy blur filters to prevent main-thread repaint lag */}
      <div style={{ position:'absolute',top:'-10%',left:'-15%',width:'55%',height:'70%',borderRadius:'50%',background:'radial-gradient(circle,rgba(80,40,160,0.55) 0%,transparent 70%)',filter:'blur(40px)',transform:'translateZ(0)',willChange:'transform, filter',pointerEvents:'none'}}/>
      <div style={{ position:'absolute',bottom:'-10%',right:'-10%',width:'50%',height:'60%',borderRadius:'50%',background:'radial-gradient(circle,rgba(180,130,20,0.3) 0%,transparent 70%)',filter:'blur(50px)',transform:'translateZ(0)',willChange:'transform, filter',pointerEvents:'none'}}/>
...
function AuthenticatedApp({ user, displayName, city, identity, view, setView, onLogout }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: T.dark }}>
      ...
      {/* ⚡ Bolt: Hardware accelerate heavy backdrop-filter to prevent main-thread repaint lag */}
      <nav style={{
