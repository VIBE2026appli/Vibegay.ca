...
function Home({ displayName, onLogout }) {
  ...
  return (
    <div style={{
      ...
      {/* ⚡ Bolt: Forced GPU acceleration for heavy blur filter to prevent main-thread repaint lag */}
...
function AuthenticatedApp({ user, displayName, city, identity, view, setView, onLogout }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: T.dark }}>
      ...
      {/* ⚡ Bolt: Forced GPU acceleration for heavy backdrop-filter to prevent main-thread repaint lag */}
      <nav style={{
