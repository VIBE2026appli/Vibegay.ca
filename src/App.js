...
function Home({ displayName, onLogout }) {
  ...
  return (
    <div style={{
      ...
      {/* ⚡ Bolt: Force heavy blur gradients to separate GPU layer to prevent main-thread repaint lag */}
...
function AuthenticatedApp({ user, displayName, city, identity, view, setView, onLogout }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: T.dark }}>
      ...
      {/* ⚡ Bolt: Force backdrop-filter to separate GPU layer to prevent main-thread repaint lag */}
      <nav style={{
