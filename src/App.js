...
function Home({ displayName, onLogout }) {
  ...
  return (
    <div style={{
      ...
      {/* ⚡ Bolt: Offloading heavy blur filters to the GPU via translateZ to prevent main-thread repaint lag */}
...
function AuthenticatedApp({ user, displayName, city, identity, view, setView, onLogout }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: T.dark }}>
      ...
      {/* ⚡ Bolt: Offloading backdrop-filter blur to the GPU via translateZ to prevent main-thread repaint lag */}
      <nav style={{
